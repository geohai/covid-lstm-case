...
class covidLSTMQuantiles():
    ...
    def predict(self, X_ts, test_info_df):
        predictions_test = self.model.predict(x=X_ts)
        print("ANY NANs:", np.isnan(np.array(predictions_test)).any())
        predictions_test = np.transpose(np.array(predictions_test))[0]
        assert predictions_test.shape[0]==test_info_df.shape[0]
    ...
    def save_results(self, current_date, target_date, test_horizon,
        ...
        if not os.path.isfile(results_file):
            ...
            for quantile in self.quantiles:
                ...
                if results_df["q_"+quantile+"_pred_cases"].isnull().values.any():
                    results_df["q_"+quantile+"_pred_cases"].fillna(0,
                                                                   inplace=True)
                if results_df["q_"+quantile+"_pred_cases"].isnull().values.any():
                    sys.exit()
...
def organize_csv_for_forecast_hub(current_date, test_horizon, lag,
                                  county_skip_list=[], save=True):
    quantile_list = [25, 100, 250, 500, 750, 900, 975]
    if type(county_skip_list)!=list:
        county_skip_list = [county_skip_list]
    if county_skip_list:
        county_skip_list = [int(c) for c in county_skip_list]
    ...
    for county in counties_list:
        if county in county_skip_list:
            print("skipping county "+str(county))
            continue
        ...
        for i, quantile in enumerate(quantile_list):
            ...
            if math.isnan(pred_value):
                pred_value = 0
            if pred_value < last_pred_value:
                pred_value = last_pred_value
                print("---------------------------------------------")
                print("County:", county)
...
def check_submission_file(forecast_date):
    data = pd.read_csv("../results/"+forecast_date+"-CUBoulder-COVIDLSTM.csv")
    locations = np.unique(data.location.values).tolist()
    for loc in locations:
        county_data = data[data.location==loc]
        for target in range(1,5):
            county_horizon_data = county_data[county_data.target==str(target)+" wk ahead inc case"]
            county_horizon_data = county_horizon_data[county_horizon_data.type=="quantile"]
            previous_value = 0
            for row in range(county_horizon_data.shape[0]):
                pred = county_horizon_data.value.values[row]
                if pred < previous_value:
                    print("-----------------------------------------------")
                    print("csv has decreasing values in quantiles")
                    print("County: ", loc, "Horizon: ", target)
                    print("Pred: ", pred, "Previous: ", previous_value)
                    sys.exit()
                previous_value = pred


if __name__ == '__main__':
    organize_csv_for_forecast_hub("2022-01-09", 1, county_skip_list=["1001", "1005"])
