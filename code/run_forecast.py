...
def parse_args():
    ...
    parser.add_argument("-l", "--lag", type=int, default=4,
    ...
    parser.add_argument("-sk", "--skipcounty", nargs="+", type=int, default=[],
                        help="counties to skip")
...
def main():
    ...
    results_q_folder = "../results/quantile_predictions/"
    if not os.path.isdir(results_q_folder):
        os.mkdir(results_q_folder)
    ...
    for hz in range(1,5):
        ...
        organize_csv_for_forecast_hub(current_date=args.date,
                                      test_horizon=hz,
                                      lag=args.lag,
                                      county_skip_list=args.skipcounty)
    check_submission_file(args.date)
